...
export default function StudyForm() {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    if (!dateTime || !topic || !duration) {
      setError("Please fill in date/time, topic, and duration.");
      return;
    }
    ...
    try {
      ...
      alert(`Study logged: ${topic} for ${durationMinutes} minutes.`);
  ...
  return (
    <form onSubmit={handleSubmit} class="space-y-4 p-6 border rounded-lg shadow-md bg-white dark:bg-gray-800 max-w-md mx-auto">
      <h2 class="text-2xl font-semibold mb-4 text-center text-gray-800 dark:text-gray-200">Log Study Session</h2>
      {error && (
      ...
      <div>
       ...
       <div>
        <label htmlFor="topic" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Topic / Subject:</label>
        <input
          type="text"
          id="topic"
          name="topic"
          ...
          disabled={disabled}
          required
          placeholder="e.g., English Vocabulary, Deno Basics"
          class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 disabled:opacity-50"
        />
      </div>
       <div>
       ...
       <div>
        ...
        <textarea
          ...
          disabled={disabled}
          class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 disabled:opacity-50"
          placeholder="Specific topics covered, difficulties, etc."
        />
      </div>
